...
public class PermissionRequester {
    ...
    public static final int NOT_SUPPORT_VERSION = 2;   // 요청 Android OS Version이 마쉬멜로우 미만일 경우

    public static final int ALREADY_GRANTED = -1;       // 이미 요청 권한 가지고 있을 경우

    public static final int REQUEST_PERMISSION = 0;   // 권한 System에게 요청한 경우 (Activity의 onRequestPermissionResult()로 결과 return
    ...
    public int request(final String permission, final int requestCode, final OnClickDenyButtonListener denyAction) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            */
            ...
            int permissionCheck = ContextCompat.checkSelfPermission(context, permission); // 해당 권한이 여부 확인
