...
public class LoginActivity extends AppCompatActivity implements OnClickListener {  // "로그인" 화면
    ...
    /**
     * 버튼에 따른 기능 부여
     * "R.id.login_layout" -> "로그인" Button        (로그인 성공 시, 메인 화면으로 이동)
     * "R.id.join_layout" -> "회원가입" Button        (회원가입 창으로 이동)
     * "R.id.findId_layout" -> "아이디 찾기" Button   (ID 찾기 창으로 이동)
     * "R.id.findPw_layout" -> "비밀번호 찾기" Button (Password 찾기 창으로 이동)
     *
     * @param view
     */
    @Override
